...
module.exports = {
    ...
    loadMiddleware(opt) {
        ...
        // middlewares 这个定义就是this.loadToApp 传进去的第二参数
        // 2020-11-08 11:41:07 定义的app.middleware貌似没有什么作用吧，中间件函数安装后，没有哪里还需要使用
        for (const name in app.middlewares) {
            debug('After this.loadToApp, app.middlewares %s', name);
        ...
        for (const name of middlewareNames) {
            ...
            let mw = app.middlewares[name]; //取到这个中间件的定义函数
            mw = mw(options, app); // 传递参数给中间件，返回一个真正的中间件函数
            ...
            if (mw) {
                ...
                app.use(mw); // 真正使用中间件的地方，底层就是用的koa的接口
