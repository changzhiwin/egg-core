...
class EggLoader {
    ...
    /**
     * 获取当前用户的home目录，例如 /Users/changzhi
     * Get home directory
    ...
    /**
     * 为什么称之为框架的框架了？这个函数起到很关键的作用：
     * 对于继承EggCore的子类，依次向上加载直到到达EggCore本身(不包括)
     * 每个子类都需要定义'egg#eggPath'，这个地方是加载的basePath
    ...
    getEggPaths() {
        ...
        let proto = this.app; //EggCore本身是最底层的框架，没有定义'egg#eggPath'的
        ...
        while (proto) {
            ...
            // 2020-11-02 11:19:36 返回的目录是代码app子目录的上层目录
            ...
            if (!eggPaths.includes(realpath)) {
                eggPaths.unshift(realpath); // 这里用的是unshift，是因为后面用的时候顺序要求：EggCore -> 框架 -> 框架 -> 应用
            }
        }
    ...
    loadFile(filepath, ...inject) {
        ...
        // require 了这个 filepath 指向的js文件
        let ret = this.requireFile(filepath);
        if (is.function(ret) && !is.class(ret)) {
            ret = ret(...inject); // 2020-11-10 10:58:05 router.js是一个函数式声明： (app) => app.router.get('/', controller.home.index)
        }
        return ret;
    }
    ...
    requireFile(filepath) {
        ...
        const ret = utils.loadFile(filepath); // 这里就是require(filepath)
    ...
    getLoadUnits() {
        ...
        // 这个函数，会被xxLoader调用多次，不缓存起来我理解可能是只是初始化用，导致意义不大？
        // 2020-11-03 16:03:25 眼瞎，this.dirs就是缓存呀
        ...
        // 2020-10-26 16:56:46 在egg的config/plugin.js里面，配置了插件
        // 2020-10-26 20:47:57 this.orderPlugins 这个属性是在loadPlugin里面计算出来的
     ...
     * property这个参数就是说，这个属性叫什么名字，本质就是 app[property]
    ...
    //filename的值可能是config/plugin
    // eg.  config.default.js
...
for (const loader of loaders) {
    Object.assign(EggLoader.prototype, loader); // 直接在原型上添加方法
}
