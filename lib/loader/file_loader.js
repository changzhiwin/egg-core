 ...
 * 这个Class解决的问题是：将指定目录下的文件（包括子目录）所有文件导出的内容，以路径+文件名为属性组织在target上
 * @since 1.0.0
 */
class FileLoader {
        ...
        for (const item of items) {
            ...
            item.properties.reduce((target, property, index) => {
                ...
                if (index === item.properties.length - 1) {
                    ...
                    // 这里没有new，如果是export的class，应该是不行的。
                    // 2020-11-08 11:46:01 对于middleware来说，导出函数就够了
                    // 2020-11-09 20:52:48 对于contorl在getExports的时候，使用了initializer，实例化了
                    obj = item.exports;
                    if (obj && !is.primitive(obj)) {
                        obj[FULLPATH] = item.fullpath;
                        obj[EXPORTS] = true; // 这个值在context_loader里面使用了，getInstance里面判断是否需要new Class
                    }
                } else {
                    obj = target[property] || {};
                }
    ...
    parse() {
        ...
        if (!files) {
            files = (process.env.EGG_TYPESCRIPT === 'true' && utils.extensions['.ts']) ? ['**/*.(js|ts)', '!**/*.d.ts'] : ['**/*.js'];
        } else {
        ...
        for (const directory of directories) {
            const filepaths = globby.sync(files, { cwd: directory }); // 在一个目录匹配符合条件后缀的所有文件（包括子目录）
            for (const filepath of filepaths) {
                const fullpath = path.join(directory, filepath); // 获得绝对路径
...
function getExports(fullpath, { initializer, call, inject }, pathName) {
    let exports = utils.loadFile(fullpath); // 等同于 require(fullpath)
    // process exports as you like
    // 例如control的loader是定义了initializer的
    if (initializer) {
