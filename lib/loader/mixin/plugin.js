...
module.exports = {
   ...
   * @since 1.0.0
   * 找出所有声明的插件
   * 核心输出了2个属性：this.orderPlugins、this.plugins，其实这两个属性本质一样：一个数组表示、一个k-v表示
   */
  loadPlugin() {
    ...
    // 读取框架定义的plugin.js
    // debug('Loaded app plugins, eggPath = %j', this.eggPaths);
    ...
    this.allPlugins = {};
    // 2020-10-26 17:38:14 会有下面三种plugin配置的来源
    ...
    // 2020-10-26 17:42:39 这是有顺序策略吗？
    // 2020-10-26 20:40:22 解决依赖问题，所以有先后顺序；有个递归调用的算法在里头
     ...
     * @since 1.0.0
     */
    this.plugins = enablePlugins; // 所有需要加载的plugin，包含显式声明和隐式依赖
   ...
   * 读取plugin的配置文件，入参为一个数组；出参为所有配置路径合并后的配置对象
   */
  ...
  // 从所有插件中匹配enble的插件，并确定插件依赖顺序，解决加载可能嵌套的问题。这里会引申出一个问题：因为依赖加载，会隐式加载没有声明的插件
  getOrderPlugins(allPlugins, enabledPluginNames, appPlugins) {
    ...
    // 2020-10-26 17:58:57 这里涉及一个算法，插件可能有嵌套依赖，这个函数应该是检查依赖，确定加载顺序；具体算法实现瞅了一眼，有递归调用
  ...
  // 用过名字查找该插件源文件的位置，会搜索App、egg目录下的node_modules
